            ...
            if (this.mapId == 140) {
                ...
                if (BossClone != null) {
                    this.createOtherMenu(player, ConstNpc.BASE_MENU,
                            ...
                            "OK");
                } else if (PotaufeuService.gI().hasUsedDailyChallenge(player)) {
                    if (PotaufeuService.gI().isAutoReturnReady(player)) {
                        PotaufeuService.gI().returnHome(player);
                        return;
                    }
                    this.createOtherMenu(player, ConstNpc.BASE_MENU,
                            "Hôm nay ngươi đã thách đấu. Capsule sẽ tự động đưa ngươi về nhà sau "
                                    + PotaufeuService.gI().secondsUntilHomeCapsule(player) + " giây.",
                            "OK");
                } else {
